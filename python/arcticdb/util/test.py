...
def merge_update(
    target: pd.DataFrame, source: pd.DataFrame, on: Optional[List[str]] = None
) -> pd.DataFrame:
    """
    ...
    # Create copies of "on" columns for indexing. MultiIndex.set_index normalizes
    # None to np.nan for object/string dtypes, so None and np.nan will match.
    # The original columns remain in the dataframe to preserve original values.
